    ...
    while True:
        with state.lock:
            ...
            for det in state.detections:
                ...
                # Use PIL to render Korean text (cv2.putText cannot handle Unicode)
                if korean_font is not None:
                    pil_img = Image.fromarray(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
                    draw = ImageDraw.Draw(pil_img)
                    tx, ty = x1, max(y1 - 36, 0)
                    # Dark background for readability
                    bbox_text = draw.textbbox((tx, ty), text, font=korean_font)
                    draw.rectangle(bbox_text, fill=(0, 0, 0, 180))
                    draw.text((tx, ty), text, font=korean_font, fill=(0, 255, 0))
